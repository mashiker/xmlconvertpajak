...
# -*- coding: utf-8 -*-
"""
Fix: Regenerate analisis_saham_jevi_2026.pdf with proper table formatting.
All table cells wrapped in Paragraph() objects to prevent raw font tags.
"""
...
pdfmetrics.registerFontFamily('TimesNewRoman', normal='TimesNewRoman', bold='TimesNewRoman')
...
# STYLES
s_cover_title = ParagraphStyle('CoverTitle', fontName='SimHei', fontSize=24, leading=32, textColor=DARK_BLUE, alignment=TA_CENTER, spaceAfter=12)
s_cover_sub = ParagraphStyle('CoverSub', fontName='SimHei', fontSize=13, leading=20, textColor=MEDIUM_BLUE, alignment=TA_CENTER, spaceAfter=8)
s_cover_info = ParagraphStyle('CoverInfo', fontName='SimHei', fontSize=11, leading=16, textColor=MEDIUM_GRAY, alignment=TA_CENTER)
s_toc1 = ParagraphStyle('TOC1', fontName='SimHei', fontSize=13, leading=22, leftIndent=20, textColor=DARK_BLUE)
s_toc2 = ParagraphStyle('TOC2', fontName='SimHei', fontSize=11, leading=18, leftIndent=40, textColor=DARK_GRAY)
s_h1 = ParagraphStyle('H1', fontName='SimHei', fontSize=18, leading=26, textColor=DARK_BLUE, spaceBefore=18, spaceAfter=10)
s_h2 = ParagraphStyle('H2', fontName='SimHei', fontSize=14, leading=20, textColor=MEDIUM_BLUE, spaceBefore=14, spaceAfter=8)
s_h3 = ParagraphStyle('H3', fontName='SimHei', fontSize=12, leading=18, textColor=DARK_GRAY, spaceBefore=10, spaceAfter=6)
s_body = ParagraphStyle('Body', fontName='SimHei', fontSize=10.5, leading=17, textColor=black, alignment=TA_LEFT, wordWrap='CJK', spaceBefore=2, spaceAfter=4)
s_bullet = ParagraphStyle('Bullet', fontName='SimHei', fontSize=10.5, leading=17, textColor=black, alignment=TA_LEFT, wordWrap='CJK', leftIndent=20, bulletIndent=8, spaceBefore=1, spaceAfter=2)
s_caption = ParagraphStyle('Caption', fontName='SimHei', fontSize=9.5, leading=14, textColor=MEDIUM_GRAY, alignment=TA_CENTER, spaceBefore=4, spaceAfter=8)
s_disc = ParagraphStyle('Disc', fontName='SimHei', fontSize=8.5, leading=13, textColor=MEDIUM_GRAY, alignment=TA_CENTER, wordWrap='CJK')
s_tag = ParagraphStyle('Tag', fontName='TimesNewRoman', fontSize=10, leading=14, textColor=ACCENT_BLUE, spaceBefore=2, spaceAfter=8, leftIndent=5)
s_verdict = ParagraphStyle('Verdict', fontName='SimHei', fontSize=11, leading=16, textColor=DARK_BLUE, alignment=TA_LEFT, wordWrap='CJK', spaceBefore=6, spaceAfter=6, leftIndent=10)
s_th = ParagraphStyle('TH', fontName='SimHei', fontSize=9.5, leading=14, textColor=white, alignment=TA_CENTER, wordWrap='CJK')
s_td = ParagraphStyle('TD', fontName='SimHei', fontSize=9, leading=14, textColor=black, alignment=TA_CENTER, wordWrap='CJK')
...
def bullet(t): return Paragraph(t, s_bullet, bulletText='\u2022')
def divider(): return HRFlowable(width="100%", thickness=1, color=LIGHT_BLUE, spaceAfter=10, spaceBefore=6)

def verdict_box(text):
    data = [[Paragraph(text, s_verdict)]]
...
def make_table(data, col_widths, caption=None):
    ...
    for ri, row in enumerate(data):
        ...
        for cell in row:
            if ri == 0:
                para_row.append(Paragraph('<b>' + str(cell) + '</b>', s_th))
            else:
                para_row.append(Paragraph(str(cell), s_td))
...
class TocDocTemplate(BaseDocTemplate):
    def __init__(self, filename, **kwargs):
        ...
        self.addPageTemplates([
            ...
            PageTemplate(id='Normal', frames=[frame_main], onPage=self._normal_page),
        ])
...
# BUILD
OUT = '/home/z/my-project/download/analisis_saham_jevi_2026.pdf'
LOGO = '/home/z/my-project/download/alatpajak_logo.png'
story = []
...
if os.path.exists(LOGO):
    logo = Image(LOGO, width=3.5*cm, height=3.5*cm)
    logo.hAlign = 'CENTER'
    story.append(logo)
    story.append(Spacer(1, 0.8*cm))
story.append(Paragraph('ANALISIS INVESTASI SAHAM INDONESIA', s_cover_title))
story.append(Spacer(1, 0.3*cm))
story.append(HRFlowable(width="60%", thickness=2, color=ACCENT_BLUE, spaceAfter=12, spaceBefore=4))
story.append(Paragraph('Rekomendasi Portofolio Jangka Panjang ' + en('(5-10 Tahun)') + ' dengan Dividen Berkelanjutan', s_cover_sub))
story.append(Spacer(1, 1.5*cm))
story.append(Paragraph('alatpajak.id ' + en('Research Division'), s_cover_info))
story.append(Spacer(1, 0.3*cm))
story.append(Paragraph('31 Maret 2026', s_cover_info))
story.append(Spacer(1, 0.3*cm))
story.append(Paragraph('Versi 1.0 - Untuk Keperluan Edukasi Investasi', s_cover_info))
...
story.append(body('Laporan ini menyajikan rekomendasi portofolio investasi saham Indonesia yang terdiri dari ' + en('7') + ' emiten unggulan dengan fokus pada dividen berkelanjutan dan pertumbuhan jangka panjang. Portofolio ini dirancang untuk investor dengan horizon investasi ' + en('5-10') + ' tahun yang menginginkan kombinasi pendapatan dividen reguler dan apresiasi modal.'))
...
story.append(body('Dengan strategi ' + en('Dollar Cost Averaging') + ' selama ' + en('3-6') + ' bulan dan reinvestasi dividen, portofolio ini berpotensi menghasilkan ' + en('total return 12-15%') + ' per tahun dalam jangka panjang, terdiri dari dividen ' + en('5.5-6.5%') + ' dan pertumbuhan modal ' + en('6-9%') + '.'))
...
story.append(body('<b>Ringkasan Alokasi:</b> Portofolio didominasi oleh sektor perbankan ' + en('(65%)') + ' yang merupakan tulang punggung ekonomi Indonesia, dilengkapi dengan telekomunikasi defensif, ' + en('consumer goods') + ', dan eksposur komoditas yang terbatas. Expected ' + en('blended yield') + ' portofolio berada di kisaran ' + en('5.5-6.5%') + ' per tahun.'))
story.append(Spacer(1, 18))

# 4. DEEP DIVE PER SAHAM
story.append(h1('4. DEEP DIVE ANALISIS PER SAHAM'))
story.append(divider())

# 4.1 BBCA
story.append(h2('4.1 ' + en('BBCA') + ' - Bank Central Asia'))
story.append(Paragraph('THE ANCHOR PORTFOLIO', s_tag))
story.append(h3('Valuasi dan Kinerja Keuangan 2025'))
...
story.append(body('Dividen final 2025 sebesar ' + en('Rp281/saham') + ', dengan total dividen sekitar ' + en('Rp336/saham') + ' (termasuk interim). Yield berada di kisaran ' + en('4-5%') + ' dengan ' + en('payout ratio 60-70%') + '. ' + en('BBCA') + ' memiliki ' + en('track record') + ' pembagian dividen yang paling konsisten di Indonesia.'))
story.append(h3('Catalyst dan Target Harga'))
...
story.append(Paragraph('THE INCOME GENERATOR', s_tag))
story.append(h3('Valuasi dan Kinerja Keuangan 2025'))
...
story.append(h3('Profil Dividen'))
story.append(body('Dividen yield ' + en('5-7%') + ' merupakan <b>tertinggi di antara Big 4 Bank</b>. ' + en('Dividend Payout Ratio / DPR: 75-85%') + ', mencerminkan komitmen ' + en('shareholder return') + ' yang kuat. Sebagai BUMN, ' + en('BBRI') + ' memiliki kebijakan dividen yang transparan dan konsisten.'))
story.append(h3('Catalyst dan Target Harga'))
...
story.append(Paragraph('THE DEFENSIVE CASH COW', s_tag))
story.append(h3('Profil Bisnis'))
story.append(body(en('Telkom Indonesia') + ' memiliki monopoli de facto pada infrastruktur fixed broadband melalui ' + en('IndiHome') + ' dan dominasi pasar seluler melalui ' + en('Telkomsel') + ' (patungan dengan Singtel). Bisnis telekomunikasi bersifat defensif karena permintaan bersifat ' + en('inelastic') + ' - konsumen tetap berlangganan bahkan saat resesi.'))
story.append(h3('Metrik Valuasi'))
...
story.append(Paragraph('THE GROWTH BANK', s_tag))
story.append(h3('Valuasi dan Kinerja Keuangan 2025'))
...
story.append(Paragraph('THE DEFENSIVE CONSUMER', s_tag))
story.append(h3('Profil Bisnis'))
story.append(body(en('Indofood CBP Sukses Makmur') + ' adalah perusahaan ' + en('consumer goods') + ' terbesar Indonesia, menguasai pasar mi instan (Indomie), snack, dairy, minuman, dan penyedap makanan. ' + en('Pricing power') + ' yang kuat memungkinkan perusahaan menyesuaikan harga di tengah inflasi. Diversifikasi ekspor ke lebih dari ' + en('80') + ' negara memberikan pilar pertumbuhan tambahan.'))
story.append(h3('Metrik Valuasi'))
...
story.append(Paragraph('THE HIGH YIELD TRANSFORMER', s_tag))
story.append(h3('Transformasi Bisnis'))
story.append(body(en('Adaro Energy Indonesia') + ' sedang melakukan transformasi besar dari perusahaan batu bara menjadi perusahaan energi terintegrasi. Diversifikasi mencakup pembangunan smelter aluminium yang akan beroperasional pada 2026, serta investasi di energi hijau dan ' + en('renewable energy') + '. Arus kas dari operasi batu bara yang kuat menjadi sumber pendanaan transformasi ini.'))
story.append(h3('Metrik Valuasi'))
...
story.append(Paragraph('THE DIVERSIFIED MINING PROXY', s_tag))
story.append(h3('Profil Diversifikasi'))
story.append(body(en('United Tractors') + ' menawarkan eksposur ke sektor pertambangan yang unik melalui empat pilar bisnis: (1) Kontraktor tambang terbesar Indonesia - ' + en('Pama Persada') + ', (2) Distributor alat berat ' + en('Komatsu') + ', (3) Pertambangan emas melalui ' + en('PT Agincourt Resources') + ' (Martabe), dan (4) Pertambangan nikel melalui ' + en('Pamapersada') + '. Diversifikasi ini memberikan ' + en('mining exposure') + ' tanpa risiko murni pada satu komoditas.'))
story.append(h3('Metrik Valuasi'))
...
story.append(body('<b>Strategi:</b> Untuk mendapatkan dividen, investor harus membeli saham <b>sebelum ' + en('cum-dividend date') + '</b>. Harga saham biasanya turun sebesar dividen pada ' + en('ex-date') + ', namun dalam jangka panjang harga akan pulih. Untuk investor jangka panjang, ini bukan masalah signifikan karena fokus pada ' + en('total return') + '.'))
...
story.append(body('Alih-alih membeli semua sekaligus (' + en('lump sum') + '), gunakan strategi ' + en('DCA') + ' dengan membeli secara bertahap selama ' + en('3-6') + ' bulan. Ini mengurangi risiko ' + en('timing risk') + ' dan meratakan harga beli rata-rata.'))
...
story.append(body('Kondisi pasar saat ini menunjukkan tekanan dari ' + en('outflow') + ' asing yang memberikan valuasi yang lebih menarik. Ini merupakan ' + en('window of opportunity') + ' untuk akumulasi bertahap.'))
...
story.append(body('Strategi kunci untuk memaksimalkan ' + en('compounding effect') + ' adalah dengan mereinvestasi seluruh dividen yang diterima kembali ke portofolio. Dengan yield ' + en('5.5-6.5%') + ' dan reinvestasi, efek ' + en('compounding') + ' akan signifikan dalam ' + en('5-10') + ' tahun.'))
story.append(h2('6.4 Monitoring dan Review'))
...
# 7. RISIKO & MITIGASI
story.append(h1('7. RISIKO DAN MITIGASI'))
story.append(divider())
...
story.extend(make_table(risiko_data, [110, 70, 55, 195], 'Tabel 3: Matriks Risiko dan Mitigasi'))

story.append(body('<b>Catatan Penting:</b> Portofolio ini dirancang dengan prinsip diversifikasi untuk meminimalkan risiko spesifik emiten. Alokasi defensif sebesar ' + en('60%') + ' (bank + telco) memberikan bantalan terhadap gejolak pasar, sementara alokasi ' + en('20%') + ' pada komoditas memberikan ' + en('upside') + ' potensial.'))
story.append(Spacer(1, 18))
...
sources = [
    ...
    'Eddyelly (Konten Kreator Saham) - Analisis Teknikal dan Fundamental',
