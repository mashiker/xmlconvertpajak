...
const workflowColors: Record<string, string> = {
  bupot: 'text-emerald-600 bg-emerald-50 border-emerald-200',
  faktur: 'text-amber-600 bg-amber-50 border-amber-200',
  spt_tahunan: 'text-teal-600 bg-teal-50 border-teal-200',
  spt_masa_ppn: 'text-cyan-600 bg-cyan-50 border-cyan-200',
  bea_meterai: 'text-rose-600 bg-rose-50 border-rose-200',
  daftar_harta: 'text-violet-600 bg-violet-50 border-violet-200',
};
    ...
    >
      ...
      <div className="text-center space-y-4 relative">
        ...
        <motion.p
          ...
          className="text-slate-500 text-sm lg:text-base max-w-lg mx-auto"
        >
          ...
          <motion.div key="tabs" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <Tabs defaultValue={categories[0].id} className="w-full">
              <div className="flex justify-center mb-6">
                <TabsList className="flex flex-wrap h-auto gap-1.5 bg-gray-50 border border-gray-200 p-2 rounded-2xl max-w-3xl">
                  {categories.map((cat) => {
                    ...
                    return (
                      <TabsTrigger
                        key={cat.id}
                        value={cat.id}
                        className="flex items-center gap-1.5 text-xs sm:text-sm data-[state=active]:bg-emerald-50 data-[state=active]:text-emerald-600 data-[state=active]:border-emerald-200 data-[state=active]:border data-[state=active]:shadow-sm rounded-lg px-3 py-2 transition-all duration-200 border border-transparent text-slate-500 hover:text-slate-700"
                      >
                        <Icon className="w-3.5 h-3.5" />
                        <span className="hidden sm:inline">{cat.name}</span>
                        <span className="sm:hidden">{cat.name.split(' ')[0]}</span>
                        <Badge variant="secondary" className="text-[10px] bg-gray-100 text-slate-500 px-1.5 py-0 border-0">
                          {count}
                        </Badge>
                      </TabsTrigger>
                    );
...
function TemplateCard({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-emerald-500/30 via-teal-500/20 to-amber-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-400" />
      <div className="absolute inset-0 rounded-2xl bg-white group-hover:bg-gray-50 transition-all duration-300" />

      <div className="relative rounded-2xl bg-white border border-gray-200 group-hover:border-emerald-200 p-4 flex gap-3.5 shadow-sm group-hover:shadow-md">
        <div className={cn(
          'flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center transition-all duration-300',
          isSelected
            ? 'bg-emerald-600 text-white shadow-sm'
            : 'bg-gray-100 text-slate-500 group-hover:bg-emerald-50 group-hover:text-emerald-600'
        )}>
          <Icon className="w-5 h-5" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <Badge variant="secondary" className="text-[10px] font-mono px-1.5 py-0 bg-gray-100 text-slate-500 border-0">
              {template.code}
            </Badge>
            {template.popular && (
              <Badge className="text-[10px] px-1.5 py-0 bg-amber-50 text-amber-600 border border-amber-200 hover:bg-amber-50">
          ...
          <p className="text-xs text-slate-500 mt-0.5 line-clamp-2 group-hover:text-slate-600 transition-colors">
            {template.description}
          </p>
        </div>
        <ChevronRight className={cn(
          'w-4 h-4 self-center flex-shrink-0 transition-all duration-300',
          isSelected ? 'text-emerald-600 translate-x-0.5' : 'text-slate-400 group-hover:text-emerald-500 group-hover:translate-x-0.5'
        )} />
      </div>
    </div>
  );
