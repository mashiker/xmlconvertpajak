    ...
    <div className="min-h-screen flex flex-col relative">
      ...
      <section className="relative z-10 pt-12 pb-8 sm:pt-16 sm:pb-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-600 text-xs font-medium mb-6">
            ...
            <p className="text-slate-500 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed">
              Pilih dari <span className="text-emerald-600 font-semibold">25 template</span> converter XML Coretax DJP.
      ...
      <section className="relative z-10 pb-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          >
            {[
              { icon: Database, label: 'Template', value: stats.templates, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-200' },
              { icon: Grid3X3, label: 'Kategori', value: stats.categories, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-200' },
              { icon: Layers, label: 'Workflow', value: stats.workflows, color: 'text-teal-600', bg: 'bg-teal-50', border: 'border-teal-200' },
              { icon: CheckCircle2, label: 'Gratis', value: '100%', color: 'text-cyan-600', bg: 'bg-cyan-50', border: 'border-cyan-200' },
            ].map((stat, i) => (
              <div
                key={i}
                className={`bg-white rounded-2xl p-4 flex items-center gap-3 ${stat.border} border shadow-sm`}
              >
      ...
      <section className="relative z-10 pb-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <p className="text-xs text-slate-500">
            Menampilkan <span className="text-emerald-600 font-medium">{filteredTemplates.length}</span> template
            {activeCategory !== 'all' && (
              <span> dalam <span className="text-slate-700">{categories.find((c) => c.id === activeCategory)?.name}</span></span>
            )}
            {searchQuery && (
              <span> untuk &ldquo;<span className="text-slate-700">{searchQuery}</span>&rdquo;</span>
            )}
          </p>
        </div>
      </section>
          ...
          <AnimatePresence mode="wait">
            {filteredTemplates.length === 0 ? (
              ...
              >
                ...
                <h3 className="text-lg font-semibold text-slate-600 mb-2">Template tidak ditemukan</h3>
                <p className="text-sm text-slate-400">Coba ubah kata kunci atau filter kategori</p>
              </motion.div>
            ) : (
              <motion.div
...
function TemplateCard({
  ...
  return (
    ...
    >
      {/* Subtle hover glow */}
      <div className="absolute inset-0 rounded-2xl bg-emerald-50/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

      <div className="relative h-full bg-white rounded-2xl border border-gray-200 group-hover:border-emerald-200 transition-colors duration-300 overflow-hidden flex flex-col shadow-sm group-hover:shadow-md">
        {/* Top accent line */}
        <div className="h-0.5 w-full bg-gradient-to-r from-transparent via-emerald-400 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        ...
        <div className="p-5 flex flex-col flex-1">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {template.popular && (
                    <span className="flex items-center gap-0.5 text-[10px] font-medium text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-md">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-3">
            {/* Category */}
            <span className="text-[10px] text-slate-500 bg-gray-100 border border-gray-200 px-2 py-0.5 rounded-md">
              {categoryObj?.name}
            </span>
            {/* Workflow type */}
            ...
            {/* Steps count */}
            <span className="flex items-center gap-1 text-[10px] text-slate-500 bg-gray-100 border border-gray-200 px-2 py-0.5 rounded-md">
          ...
          <div className="flex-1 mb-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {previewFields.map((field) => (
                <span
                  key={field.name}
                  className="text-[10px] text-slate-500 bg-gray-50 border border-gray-100 px-1.5 py-0.5 rounded"
                >
                  {field.label}
              ...
              {moreCount > 0 && (
                <span className="text-[10px] text-emerald-600 bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded">
                  +{moreCount} lainnya
                </span>
              )}
            </div>
          ...
          {/* Field counts */}
          <div className="flex items-center gap-3 text-[10px] text-slate-400 mb-4 pb-4 border-b border-gray-100">
            {headerCount > 0 && (
